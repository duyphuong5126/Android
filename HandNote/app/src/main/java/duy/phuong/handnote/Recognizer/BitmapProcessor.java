...
public class BitmapProcessor {
    ...
    public void onDetectCharacter(final Character character, Bitmap src, DetectCharactersCallback callback) {
        ...
        switch (mMode) {
            ...
            case CONTOUR:
                for (Character f : detectedBitmaps) {
                    for (Bitmap bitmap : findContour(f.mBitmap, true)) {
        ...
        if (bitmap != null) {
            ...
            for (Map.Entry<Integer, Integer> entry : listColumns.entrySet()) {
                if (entry.getKey() >= 0 && entry.getValue() >= 0) {
                    ...
                    if (horizontalHistogram.length > 0) {
                        int startRow = -1;
                        int endRow = -1;
    ...
    private ArrayList<Character> getSegments(Character character) {
        ArrayList<Character> resultBitmaps = new ArrayList<>();
        Bitmap src = copyBitmap(character.mBitmap);
        ArrayList<Bitmap> listContours = findContour(src, false);
    ...
    private ArrayList<Character> filterTrash(Bitmap bitmap, Bitmap src) {
        ...
        for (Rect r : rect) {
            if (r.width() > 0.1d * src.getWidth() && r.height() > 0.1d * src.getHeight()) {
                ...
                character.mListContours = findContour(character.mBitmap, false);
    ...
    public ArrayList<Bitmap> findContour(Bitmap bitmap, boolean summarize) {
        ...
        for (int i = 0; i < src.getHeight() && p == null; i++)
            for (int j = 0; j < src.getWidth() && p == null; j++) {
                if (src.getPixel(j, i) == Color.BLACK && !notOnBorder(new Point(j, i), src, Color.BLACK)) {
                    p = new Point(j, i);
                }
            }
        ...
        while (p != null) {
            ...
            while (!end) {
                ...
                if (x - 1 >= 0 && y + 1 < src.getHeight()) {
                    if (!notOnBorder(new Point(x - 1, y + 1), src, Color.BLACK)) {
                        p = new Point(x - 1, y + 1);
                        moved = true;
                        points.push(p);
                    }
                ...
                if (x - 1 >= 0 && !moved) {
                    if (!notOnBorder(new Point(x - 1, y), src, Color.BLACK)) {
                        p = new Point(x - 1, y);
                        moved = true;
                        points.push(p);
                    }
                ...
                if (y + 1 < src.getHeight() && !moved) {
                    if (!notOnBorder(new Point(x, y + 1), src, Color.BLACK)) {
                        p = new Point(x, y + 1);
                        moved = true;
                        points.push(p);
                    }
        ...
        if (summarize) {
            ...
            mGloBalContour.eraseColor(Color.WHITE);
            ...
            for (MyPath myPath : myPaths) {
                ...
                for (Point point : myPath.getListPoint()) {
                    ...
                    mGloBalContour.setPixel(point.x, point.y, Color.RED);
                }
        ...
        if (right >= 0) {
            ...
            if (rightFrag != null) {
                ...
                if (r != null) {
                    ...
                    f.mListContours = findContour(rightFrag, false);
    ...
    private boolean isb(Character character) {
        ...
        Character c = new Character();
        c.mBitmap = rotated;
        c.mListContours = findContour(rotated, false);
            ...
            if (list.size() == 2) {
                ...
                if (Math.abs(rSmall.top - rBig.top) > Math.abs(rSmall.bottom - rBig.bottom)
                    ...
                    if (interested != null) {
                        ...
                        f.mListContours = findContour(interested, false);
                        return isi(f) || isL(f);
                    }
                }
            }
        ...
        if (list.size() == 2) {
            ...
            if (interested != null) {
                ...
                if (start >= 0) {
                    ...
                    c.mListContours = findContour(c.mBitmap, false);
                    c.mListRectContour = new ArrayList<>();
                    if (isn(c)) {
                        return false;
                    }
                ...
                if (rect != null && rBig != null) {
                    ...
                    if (row >= 0) {
                        ...
                        if (r.size() == 1) {
                            Character c = new Character();
                            c.mBitmap = cropped;
                            c.mListContours = findContour(cropped, false);
                            ...
                            c.mListContours = findContour(interested, false);
                            return closeToBot && isV_Inverse(c);
                        }
                    }
                }
    ...
    private boolean isM(Character character) {
        ...
        if (blackLeft != null && blackRight != null) {
            if (Math.abs(blackLeft.x - blackRight.x) > 0 && src.getHeight() > 0 && blackLeft.x >= FingerDrawerView.CurrentPaintSize
                    && blackRight.x <= src.getWidth() - FingerDrawerView.CurrentPaintSize) {
        ...
        int split = -1;
        if (getColumnHeightFromTop(src, mid) > getColumnHeightFromTop(src, mid + 1)) {
        ...
        Bitmap left = cropBitmap(src, 0, 0, split, src.getHeight());
        ArrayList<Rect> r1 = detectAreasOnBitmap(left, 0, 0);
        Bitmap right = cropBitmap(src, split, 0, src.getWidth() - split, src.getHeight());
    ...
    private boolean ism(Character character) {
        ...
        if (right - 5 >= 0) {
            ...
            if (left != null && r != null) {
                ...
                fLeft.mBitmap = left;
                fLeft.mListContours = findContour(left, false);
                Character fRight = new Character();
                fRight.mBitmap = r;
                fRight.mListContours = findContour(r, false);
                if (isN(fLeft)) {
                    return true;
                }
                if (isN(fRight)) {
    ...
    private boolean isN(Character character) {
        ...
        if (right != null && left != null) {
            ...
            fRight.mListContours = findContour(right, false);
    ...
    private boolean isW(Character character) {
        ...
        if (blackLeft != null && blackRight != null) {
            ...
            if (center != null) {
                Character f = new Character();
                f.mBitmap = center;
                f.mListContours = findContour(center, false);
    ...
    private boolean is5(Character character) {
        ...
        if (startCol >= 0 && startRow >= 0) {
            ...
            boolean twoSegments = countRowSegments(point.y, src) == 2;
            boolean fat = getHorizontalHistogram(src)[point.y] >= 0.3d * src.getWidth();
            return (twoSegments || fat) && countFatRow > 0 && countThree >= 0.4d * src.getWidth();
    ...
    private boolean is9(Character character) {
        ...
        Character f = new Character();
        f.mBitmap = rotated;
        f.mListContours = findContour(rotated, false);
